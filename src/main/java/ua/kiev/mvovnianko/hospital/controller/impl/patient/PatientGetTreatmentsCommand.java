...
import static ua.kiev.mvovnianko.hospital.utils.UtilConstants.DOCTOR_PAGE;
 ...
 *
 */
public class PatientGetTreatmentsCommand implements Command {

    private static final Logger LOGGER = LogManager.getLogger(PatientGetTreatmentsCommand.class);

    private final TreatmentService SERVICE;

    public PatientGetTreatmentsCommand(TreatmentService treatmentService) {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        LOGGER.debug("Command PatientGetTreatments starts");
        ...
        if (!(sortBy.equals(MYSQL_DISEASE_NAME) || sortBy.equals(MYSQL_TREATMENT_TYPE_NAME))) {
        ...
        try {
            ...
            treatments = SERVICE.getTreatmentsPageByPatientId(sortBy, patient.getId(), (page - 1) * recordsPerPage, recordsPerPage);
            LOGGER.info("treatments list get downloaded from db");
            int noOfRecords = SERVICE.countTreatmentsByPatientId(patient.getId());
        ...
        LOGGER.debug("Command PatientGetTreatments finished");
        return PATIENT_PAGE;
    }
}
